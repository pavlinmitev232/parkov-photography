...
export const siteSettingsSchema = z.object({
  ...
  addressBg: z.string().trim().max(240).default(""),
  addressEn: z.string().trim().max(240).default(""),
  mapQuery: z.string().trim().max(300).default(""),
  showMap: z.boolean().default(false),
  announcementEnabled: z.boolean().default(false),
  announcementTextBg: z.string().trim().max(240).default(""),
  announcementTextEn: z.string().trim().max(240).default(""),
